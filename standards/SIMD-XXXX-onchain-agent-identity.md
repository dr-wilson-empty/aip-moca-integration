...
> **Editor's Note.** The `simd` field above is a placeholder. Before submitting the
> PR to `solana-foundation/solana-improvement-documents`, replace `XXXX` with the
> next unallocated four-digit number visible on the `main` branch of that repository.
